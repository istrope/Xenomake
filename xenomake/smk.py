...
from xenomake.config import ConfigFile, setup_spatial_parser, ConfigMainVariable, RunMode,Spatial_Setup
...
def setup_init_parser(parent_parser):
    ...
    parser_init.add_argument(
        '--spatial_mode',
        help='use preset run modes to run samples based upon technology used [custom,visium,slide_seq,hdst_seq,stero_seq,pixel_seq,dbit_seq]',
        required=True,
        default=None,
        type=str
    )
...
def xenomake_init(args):
    ...
    cf.variables['sample'] = args['sample']
    cf.variables['project'] = args['project']
    cf.variables['r1'] = args['r1']
    cf.variables['r2'] = args['r2']
    cf.variables['run_mode'] = args['run_mode']
    cf.variables['root_dir'] = args['root_dir']
    cf.variables['temp_dir'] = args['temp_dir']
    cf.variables['dropseq'] = args['dropseq_tools']
    cf.variables['picard'] = args['picard']
...
def process_run_mode(args):
    ...
    if args['run_mode'] == 'custom':
        ...
        #set custom args
        cf.variables['ambiguous'] = args['ambiguous']
        cf.variables['mm_reads'] = args['mm_reads']
        cf.variables['downstream'] = args['downstream']
        cf.variables['genic_only'] = args['genic_only']
    
    elif args['run_mode'] == 'lenient':
        cf.variables['ambiguous'] = True
        cf.variables['mm_reads'] = True
        cf.variables['downstream'] = True
        cf.variables['genic_only'] = True

    elif args['run_mode'] == 'prude':
        cf.variables['ambiguous'] = False
        cf.variables['mm_reads'] = False
        cf.variables['downstream'] = True
        cf.variables['genic_only'] = True
