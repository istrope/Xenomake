...
if args.mouse_ref.endswith(('fa','fasta','fna','fna.gz','fa.gz','fasta.gz')) == False:
    raise Exception('mouse assembly must be a fasta genome reference file')

if args.human_ref.endswith(('fa','fasta','fna','fna.gz','fa.gz','fasta.gz')) == False:
...
if (args.xengsort_hash is not None) & (args.xengsort_info is not  None):
...
if args.human_index is not None:
...
if args.mouse_index is not None:
    check_index(args.mouse_index)
    os.system('ln --force --symbolic "$(readlink --canonicalize %s)" species/mouse_star_index' % args.mouse_index)
